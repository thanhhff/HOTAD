    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            if pn.endswith('bias'):
                ...
                no_decay.add(fpn)
            elif 'text_model' in pn or 'txt_projection' in pn or 'vid_projection' in pn:
                decay.add(fpn)
...
def train_one_epoch(
    ...
    start = time.time()
    for iter_idx, video_list in tqdm(enumerate(train_loader, 0)):
        # zero out optim
        optimizer.zero_grad(set_to_none=True)
        optimizer_txt.zero_grad(set_to_none=True)
        # forward / backward the model
        losses, fpn_feats_for_text = model(video_list)
        txt_losses = model_txt(video_list, data_label_str, fpn_feats_for_text, combine_mode=combine_mode)

        # check txt_losses is nan
        if torch.isnan(txt_losses):
            print("txt_losses is nan")
            import pdb; pdb.set_trace()

        total_losses = losses['final_loss'] + txt_losses
...
def valid_one_epoch(
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        with torch.no_grad():
            output, fpn_feats_for_text = model(video_list)
            scores, preds = model_txt(video_list, data_label_str, fpn_feats_for_text, output, mode='eval', combine_mode=combine_mode)

            for i in range(len(output)):
                output[i]['labels'] = preds[:len(output[i]['labels'])].cpu()
                preds = preds[len(output[i]['labels']):]
