...
class PtTransformer(nn.Module):
        ...
        if backbone_type == 'convTransformer':
            ...
            self.backbone = make_backbone(
                ...
                **{
                    'n_in': input_dim,
                    'n_embd': embd_dim,
                    'sgp_mlp_dim': 768, #THUMOS
                    # 'sgp_mlp_dim': 2048, #ACTIVITYNET
                    'n_embd_ks': embd_kernel_size,
                    'max_len': max_seq_len,
                    'arch': backbone_arch,
                    'scale_factor': scale_factor,
                    ...
                    'k': 5, # THUMOS
                    # 'k': 1.3, # ACTIVITYNET
                    'init_conv_vars': 0 # THUMOS
                    # 'init_conv_vars': 0.2 # ACTIVITYNET
                }
            )
        else:
            self.backbone = make_backbone(
    ...
    def forward(self, video_list):
        ...
        if self.training:
            ...
            gt_labels = [torch.zeros_like(x['labels']).to(self.device) for x in video_list]
            ...
            return losses, fpn_feats_for_text
            ...
            return results, fpn_feats_for_text
    ...
    def preprocessing(self, video_list, padding_val=0.0):
        ...
        for i in range(len(feats)):
            if torch.isnan(feats[i]).any():
                feats[i][torch.isnan(feats[i])] = 0
